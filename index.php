...
// Fungsi untuk mendekripsi data yang telah dienkripsi menggunakan AES
...
if(isset($_POST['submit']) && $_FILES['gambar']['size'] > 0){
    ...
    if(in_array($ext, $valid_array)){
        ...
        // Baca file gambar dan enkripsi isinya menggunakan AES
        $file_contents = file_get_contents('gambar/'.$_FILES['gambar']['name']);
        $enkripsi = encryptAES($file_contents, '227006097');
        ...
        echo "<br><div class='alert alert-danger'><strong>Maaf... file yang Anda pilih bukan file gambar. Hanya file JPG, PNG, GIF, BMP atau PSD yang boleh diupload..!</strong></div>";
    }
}
...
if(isset($_POST['submit'])){
    $enkripsi = @$_POST['teks']; // Data terenkripsi dalam base64

    // Dekripsi data menggunakan AES
    $teks = decryptAES($enkripsi, '227006097');

    // Encode kembali teks yang didekripsi ke format base64
    $base64_encoded = base64_encode($teks);

    ?>
    <br>
    <img style="" border="0" src="data:image/jpeg;base64,<?php echo $base64_encoded;?>" class="img-thumbnail img-responsive">
    </div>
    <?php
}
?>
